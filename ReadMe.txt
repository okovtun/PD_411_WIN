...
TODO:
1. Шрифты в выпадающем списке должны быть отсортированы в алфавитном порядке, независимо от расширения файла;
2. Сделать длительную проверку часов (желательно больше часа или нескольких часов) С КАСТОМНЫМ ШРИФТОМ;
3. Окна выбора цвета и шрифта всегда должны открываться рядом с часами, а не в другом конце экрана.
4. В контекстное меню доьавить пункт с галочкой "Загружать при старте Windows";
5. Конфигурация программы (Topmost, Show Date/Weekday, поциция на экране, Цвета и шрифт) 
   должна сохраняться при выходе, и загружаться при следующем запуске;
6. Обеспечить возможность перемещать часы по экрану без отображения элементов управления;
